\documentclass[aspectratio=169]{beamer}
\usetheme{metropolis}
\AtBeginSection{}
\usepackage[utf8]{inputenc}
\usepackage[spanish]{babel}
\usepackage{amsmath, amssymb}
\usepackage{graphicx}
\usepackage{tikz}
\usetikzlibrary{shapes.geometric, arrows.meta, positioning, backgrounds}

\setbeamerfont{frametitle}{size=\large}

\title{Fusión de Atención Cruzada para Segmentación del Segmento Anterior}
\subtitle{Usando Representaciones de Imagen Complementarias}
\author[Alfaro · Avilés · Duque]{Caleb Alfaro \and Desireé Avilés \and Danilo Duque}
\institute{Instituto Tecnológico de Costa Rica}
\date{Semana 17, 2026}

% ─────────────────────────────────────────────────────────────────────────────
\begin{document}

% ── Secciones ────────────────────────────────────────────────────────────────
\section{Motivación}
\section{Fundamentos}
\section{Trabajo Relacionado}
\section{Método Propuesto}
\section{Experimentos y Resultados}
\section{Conclusión}

% ══════════════════════════════════════════════════════════════════════════════
%  Slide 1: Título
% ══════════════════════════════════════════════════════════════════════════════
\begin{frame}
\titlepage
\end{frame}

% ══════════════════════════════════════════════════════════════════════════════
%  Slide 2: Agenda
% ══════════════════════════════════════════════════════════════════════════════
\begin{frame}{Agenda}
\tableofcontents
\end{frame}

% ══════════════════════════════════════════════════════════════════════════════
%  MOTIVACIÓN — Slides 3–5
%  (Primero el problema clínico, luego por qué es difícil, luego el enfoque)
% ══════════════════════════════════════════════════════════════════════════════

% ── Slide 3: El problema clínico ─────────────────────────────────────────────
\begin{frame}{El Problema: Cataratas}
\begin{itemize}
    \item Las cataratas afectan a más de \textbf{100 millones de personas} en el mundo
    \item Es la \textbf{principal causa de ceguera prevenible}
    \item El diagnóstico requiere identificar estructuras del segmento anterior:
    \begin{itemize}
        \item Córnea, pupila, cristalino
    \end{itemize}
\end{itemize}
\vspace{0.5em}
\begin{block}{Necesidad clínica}
Segmentar automáticamente estas estructuras para graduación, planificación quirúrgica y guía intraoperatoria.
\end{block}
\end{frame}

% ── Slide 4: Por qué RGB no es suficiente ────────────────────────────────────
\begin{frame}{¿Por qué RGB solo no es suficiente?}
\begin{columns}
\column{0.55\textwidth}
\begin{itemize}
    \item La opacificación del cristalino genera \textbf{bajo contraste de color}
    \item Las lámparas de hendidura producen \textbf{artefactos de iluminación}
    \item Los modelos unimodales fallan precisamente cuando el paciente está más grave
\end{itemize}
\vspace{0.5em}
\begin{alertblock}{Problema clave}
El color solo no alcanza para delinear los bordes del cristalino opacificado.
\end{alertblock}
\column{0.42\textwidth}
\centering
\includegraphics[width=\textwidth]{../paper/figures/qualitative_predictions.pdf}
\end{columns}
\end{frame}

% ── Slide 5: ¿Qué es segmentación multimodal? ────────────────────────────────
\begin{frame}{¿Qué es Segmentación Multimodal?}
\begin{block}{Idea central}
Usar \textbf{múltiples representaciones} de la misma escena y \textbf{fusionarlas} para que el modelo vea más de lo que cualquier vista individual puede capturar.
\end{block}
\vspace{0.6em}
\begin{columns}
\column{0.48\textwidth}
\textbf{Multimodal clásico} (sensores distintos):
\begin{itemize}
    \item MRI T1 + T2 + FLAIR para tumores cerebrales
    \item RGB + profundidad para escenas 3D
\end{itemize}
\column{0.48\textwidth}
\textbf{Nuestro enfoque} (misma imagen, dos vistas):
\begin{itemize}
    \item Imagen RGB $\rightarrow$ features fotométricas
    \item Mapa de bordes Canny $\rightarrow$ features geométricas
    \item Espacios de features heterogéneos $\Rightarrow$ fusión multimodal
\end{itemize}
\end{columns}
\end{frame}

% ══════════════════════════════════════════════════════════════════════════════
%  FUNDAMENTOS — Slides 6–10
%  (Ahora el audience sabe POR QUÉ necesita entender estas herramientas)
% ══════════════════════════════════════════════════════════════════════════════

% ── Slide 6: ¿Qué es U-Net? ──────────────────────────────────────────────────
\begin{frame}{¿Qué es U-Net?}
\begin{columns}[T]
\column{0.58\textwidth}
Arquitectura \textbf{encoder-decoder} para segmentación:
\begin{itemize}
    \item \textbf{Encoder}: reduce resolución, aumenta canales. Captura \textbf{qué} hay.
    \item \textbf{Decoder}: aumenta resolución, reduce canales. Recupera \textbf{dónde}.
    \item \textbf{Skip connections}: conectan encoder y decoder. \textbf{Preservan el detalle fino} que se pierde al submuestrear.
\end{itemize}

\column{0.38\textwidth}
\centering
\begin{tikzpicture}[font=\tiny, scale=0.55, transform shape]
    \fill[blue!15, rounded corners=3pt] (0,0) rectangle (1.5,3);
    \fill[red!15, rounded corners=3pt] (3,0) rectangle (4.5,3);
    \fill[gray!15, rounded corners=3pt] (1.5,1.2) rectangle (3,1.8);
    \node[rotate=90] at (0.75,1.5) {\textbf{Encoder}};
    \node[rotate=-90] at (3.75,1.5) {\textbf{Decoder}};
    \node at (2.25,1.5) {\textbf{Botella}};
    \draw[blue!60, dashed, thick] (1.5,2.8) -- (3,2.8);
    \draw[blue!60, dashed, thick] (1.5,2.0) -- (3,2.0);
    \draw[blue!60, dashed, thick] (1.5,0.8) -- (3,0.8);
    \draw[-{Stealth}, thick] (0,0) -- (0,3);
    \draw[-{Stealth}, thick] (1.5,1.5) -- (3,1.5);
    \draw[-{Stealth}, thick] (4.5,3) -- (4.5,0);
    \node[align=center] at (0,-0.5) {Imagen\\$512^2$};
    \node[align=center] at (4.5,-0.5) {Máscara};
\end{tikzpicture}
\end{columns}
\begin{block}{Clave}
Usamos U-Net como backbone porque sus skip connections preservan los bordes anatómicos finos para segmentar córnea y cristalino.
\end{block}
\end{frame}

% ── Slide 7: ¿Qué es Canny? ──────────────────────────────────────────────────
\begin{frame}{¿Qué es el detector de bordes Canny?}
Transforma una imagen a color en un \textbf{mapa de bordes}.
\begin{enumerate}
    \item \textbf{Suavizar}: filtro Gaussiano para eliminar ruido.
    \item \textbf{Gradiente}: en cada píxel, qué tan rápido cambia el brillo.
    \item \textbf{Adelgazar}: solo los máximos locales en la dirección del gradiente.
    \item \textbf{Conectar bordes} con dos umbrales ($t_1=50$, $t_2=150$):
    \begin{itemize}
        \item $> t_2$: borde fuerte (se queda)
        \item $< t_1$: se descarta
        \item Entre $t_1$ y $t_2$: solo si conecta a un borde fuerte
    \end{itemize}
\end{enumerate}
\vspace{0.3em}
{\small Dos umbrales producen bordes \textbf{limpios y conectados}: eliminan ruido pero mantienen contornos relevantes.}
\end{frame}

% ── Slide 8: ¿Qué es Atención? ───────────────────────────────────────────────
\begin{frame}{¿Qué es Atención (Attention)?}
\begin{columns}[T]
\column{0.55\textwidth}
Mecanismo que permite a un modelo \textbf{enfocarse} en las partes relevantes.

\textbf{Tres componentes:}
\begin{itemize}
    \item \textbf{Q (Query)}: ``¿qué información necesito?''
    \item \textbf{K (Key)}: ``¿qué información tengo?''
    \item \textbf{V (Value)}: ``el contenido que voy a entregar''
\end{itemize}
\vspace{0.3em}
\textbf{Cálculo:} se compara Q con cada K, se obtiene un peso de relevancia y se combinan los V.

\column{0.42\textwidth}
\begin{equation*}
\text{Atención}(Q,K,V) = \text{softmax}\!\left(\frac{QK^\top}{\sqrt{d}}\right)V
\end{equation*}
\end{columns}
\vspace{0.3em}
\begin{block}{Analogía}
Como buscar en Google: tu \textbf{consulta} (Q) se compara con \textbf{claves} (K) de páginas web y recibes los \textbf{contenidos} (V) más relevantes.
\end{block}
\end{frame}

% ── Slide 9: Self-Attention vs Cross-Attention ───────────────────────────────
\begin{frame}{Self-Attention vs Cross-Attention}
\begin{columns}[T]
\column{0.48\textwidth}
\centering
\textbf{Self-Attention}
\begin{tikzpicture}[font=\footnotesize]
    \node[draw, rounded corners, fill=blue!20, minimum width=2.2cm, minimum height=0.5cm] (q) {Q};
    \node[draw, rounded corners, fill=blue!20, minimum width=2.2cm, minimum height=0.5cm, below=0.2 of q] (k) {K};
    \node[draw, rounded corners, fill=blue!20, minimum width=2.2cm, minimum height=0.5cm, below=0.2 of k] (v) {V};
    \node[left=0.2 of q, font=\footnotesize] {Misma entrada};
    \draw[dashed] (q.west) -- ++(-0.6,0);
    \node[draw, rounded corners, fill=gray!20, minimum width=2.2cm, minimum height=0.5cm, below=0.3 of v] (out) {Salida};
    \draw[->] (v) -- (out);
    \node[font=\tiny, align=center, below=0.1 of out] {Ej: ``I love cats'' $\rightarrow$\\cada palabra se atiende a sí misma};
\end{tikzpicture}

\column{0.48\textwidth}
\centering
\textbf{Cross-Attention}
\begin{tikzpicture}[font=\footnotesize]
    \node[draw, rounded corners, fill=blue!20, minimum width=2.2cm, minimum height=0.5cm] (q) {Q};
    \node[draw, rounded corners, fill=red!20, minimum width=2.2cm, minimum height=0.5cm, below=0.2 of q] (k) {K};
    \node[draw, rounded corners, fill=red!20, minimum width=2.2cm, minimum height=0.5cm, below=0.2 of k] (v) {V};
    \node[left=0.2 of q, font=\footnotesize] {Entrada A};
    \node[left=0.2 of k, font=\footnotesize] {Entrada B};
    \draw[dashed] (q.west) -- ++(-0.6,0);
    \draw[dashed] (k.west) -- ++(-0.6,0);
    \node[draw, rounded corners, fill=gray!20, minimum width=2.2cm, minimum height=0.5cm, below=0.3 of v] (out) {Salida};
    \draw[->] (v) -- (out);
    \node[font=\tiny, align=center, below=0.1 of out] {Ej: nuestro modelo\\RGB $\rightarrow$ Q, Bordes $\rightarrow$ K,V};
\end{tikzpicture}
\end{columns}
\vspace{0.3em}
\begin{alertblock}{En nuestro modelo}
El encoder RGB genera \textbf{Q}, el encoder de bordes genera \textbf{K y V}. Los features RGB ``preguntan'' dónde están los bordes y enriquecen su representación.
\end{alertblock}
\end{frame}

% ══════════════════════════════════════════════════════════════════════════════
%  TRABAJO RELACIONADO — Slides 10–12
% ══════════════════════════════════════════════════════════════════════════════

% ── Slide 11: Trabajo Relacionado — Backbone ──────────────────────────────────
\begin{frame}{Trabajo Relacionado: Segmentación Médica}
\begin{itemize}
    \item \textbf{U-Net} \textit{[Ronneberger 2015]} — arquitectura encoder-decoder con skip connections
    \begin{itemize}
        \item Backbone de nuestro modelo
        \item Preserva detalle espacial fino
    \end{itemize}
    \vspace{0.4em}
    \item \textbf{CaDIS} \textit{[Grammatikopoulou 2021]} — benchmark de escenas quirúrgicas de cataratas
    \begin{itemize}
        \item Motivación del dominio
    \end{itemize}
    \vspace{0.4em}
    \item \textbf{MedSAM / SAM-Adapter} \textit{[Ma 2024, Chen 2023]} — modelos fundacionales
    \begin{itemize}
        \item Requieren 20+ GPUs A100 y grandes volúmenes de datos
        \item \textbf{No son viables} para datasets pequeños como el nuestro
    \end{itemize}
\end{itemize}
\end{frame}

% ── Slide 12: Trabajo Relacionado — Fusión ────────────────────────────────────
\begin{frame}{Trabajo Relacionado: Fusión Multimodal}
\begin{itemize}
    \item \textbf{Vaswani 2017} — ``Attention is All You Need''
    \begin{itemize}
        \item Define el mecanismo Q/K/V que adoptamos
    \end{itemize}
    \vspace{0.3em}
    \item \textbf{mmFormer} \textit{[Zhang 2022]} — transformer multimodal médico
    \begin{itemize}
        \item Cross-attention entre modalidades de MRI
        \item \textbf{Inspiración directa de nuestro módulo de fusión}
    \end{itemize}
    \vspace{0.3em}
    \item \textbf{DECTNet} \textit{[Li 2024]} — encoder dual CNN + Transformer
    \begin{itemize}
        \item Precedente estructural: encoders paralelos con sesgos inductivos distintos
    \end{itemize}
\end{itemize}
\end{frame}

% ── Slide 13: Brecha que llenamos ─────────────────────────────────────────────
\begin{frame}{¿Dónde Encaja Nuestro Trabajo?}
\centering
\begin{tikzpicture}[font=\small, node distance=1.2cm]
    \node[draw, rounded corners, fill=blue!10, text width=3.2cm, align=center] (unet)
        {Específico para tarea\\(U-Net)\\Fuerte, pero unimodal};
    \node[draw, rounded corners, fill=orange!15, text width=3.2cm, align=center, right=1.5cm of unet] (fusion)
        {Fusión multimodal\\(mmFormer, DECTNet)\\Solo multi-sensor};
    \node[draw, rounded corners, fill=red!10, text width=3.2cm, align=center, right=1.5cm of fusion] (fm)
        {Modelos fundacionales\\(MedSAM, SAM)\\Inaccesibles en escala};
    \node[draw, rounded corners, fill=green!20, text width=3.8cm, align=center, below=1.2cm of fusion] (ours)
        {\textbf{Nuestro modelo}\\Encoder dual + cross-attention\\Liviano, sin preentrenamiento\\Fusión multimodal intra-imagen};
    \draw[-{Stealth}, thick] (unet.south) -- (ours.west);
    \draw[-{Stealth}, thick] (fusion.south) -- (ours.north);
\end{tikzpicture}
\end{frame}

% ══════════════════════════════════════════════════════════════════════════════
%  MÉTODO PROPUESTO — Slides 14–22
% ══════════════════════════════════════════════════════════════════════════════

% ── Slide 14: Vista general de la arquitectura ────────────────────────────────
\begin{frame}{Arquitectura Propuesta — Vista General}
\centering
\resizebox{0.72\textwidth}{!}{\input{../paper/figures/architecture.tikz}}
\vspace{0.3em}
\begin{columns}
\column{0.5\textwidth}\centering
{\small \textcolor{blue!60}{\rule{0.8em}{0.6em}} Encoder RGB \quad \textcolor{red!50}{\rule{0.8em}{0.6em}} Encoder bordes}
\column{0.5\textwidth}\centering
{\small \textcolor{orange!70}{\rule{0.8em}{0.6em}} Cross-Attention \quad \textcolor{purple!50}{\rule{0.8em}{0.6em}} Skip projections}
\end{columns}
\end{frame}

% ── Slide 15: Representaciones de entrada ────────────────────────────────────
\begin{frame}{Representaciones de Entrada}
\begin{columns}[T]
\column{0.5\textwidth}
\textbf{Flujo RGB}
\begin{itemize}
    \item Imagen original normalizada a $[0,1]$
    \item Redimensionada a $512 \times 512$
    \item Captura \textbf{color, textura, fotometría}
\end{itemize}
\vspace{0.3em}
\textbf{Flujo de bordes}
\begin{itemize}
    \item Filtro Canny aplicado a escala de grises
    \item Resultado replicado a 3 canales
    \item Captura \textbf{límites geométricos}
    \item \textbf{Recalculado} tras cada augmentación geométrica
\end{itemize}
\column{0.46\textwidth}
\begin{block}{La clave de la dualidad}
RGB y bordes son \textbf{espacios de features heterogéneos}: uno codifica apariencia, el otro estructura. Una sola imagen produce dos representaciones complementarias.
\end{block}
\vspace{0.3em}
\begin{alertblock}{¿Por qué recalcular Canny?}
Si la imagen se rota o voltea durante augmentación y el mapa de bordes no, el modelo aprende una inconsistencia geométrica.
\end{alertblock}
\end{columns}
\end{frame}

% ── Slide 16: Visualización Canny ────────────────────────────────────────────
\begin{frame}{¿Cómo se ve el mapa de bordes Canny?}
\centering
\includegraphics[width=0.7\textwidth]{../figures/canny_visualization.pdf}

{\small \textbf{Fila superior:} imágenes RGB \quad \textbf{Fila inferior:} bordes Canny ($t_1{=}50$, $t_2{=}150$)}\\[0.1em]
{\small Los bordes del cristalino y la córnea son geométricamente explícitos incluso cuando el contraste de color es bajo.}
\end{frame}

% ── Slide 17: Encoders duales ─────────────────────────────────────────────────
\begin{frame}{Encoders Duales}
\begin{itemize}
    \item Dos encoders U-Net \textbf{independientes} — misma arquitectura ResNet-34, \textbf{sin pesos compartidos}
    \vspace{0.3em}
    \item Cada encoder produce mapas de features en \textbf{5 resoluciones}:
    \begin{itemize}
        \item Canales: 64 → 64 → 128 → 256 → 512
        \item Resolución espacial: 256 → 128 → 64 → 32 → 16
    \end{itemize}
    \vspace{0.3em}
    \item \textbf{Encoder A} procesa la imagen RGB
    \item \textbf{Encoder B} procesa el mapa de bordes
\end{itemize}
\vspace{0.3em}
\begin{block}{Clave}
Los pesos separados permiten que cada encoder aprenda representaciones especializadas para su modalidad.
\end{block}
\end{frame}

% ── Slide 18: Módulo de Cross-Attention ──────────────────────────────────────
\begin{frame}{Módulo de Cross-Attention}
\begin{columns}
\column{0.55\textwidth}
Reshape: $(B, C, H, W) \rightarrow (B,\, HW,\, C)$

\vspace{0.4em}
\begin{equation*}
\tilde{\mathbf{F}}_A = \text{LN}\!\left(\alpha \cdot \text{MHA}(\underbrace{Q{=}\mathbf{F}_A}_{\text{RGB}},\; \underbrace{K{=}V{=}\mathbf{F}_B}_{\text{bordes}}) + \mathbf{F}_A\right)
\end{equation*}

\vspace{0.4em}
\begin{itemize}
    \item \textbf{Conexión residual} ($+ \mathbf{F}_A$): si la atención no ayuda, el flujo RGB pasa intacto
    \item \textbf{Layer Norm} estabiliza el entrenamiento
    \item \textbf{8 cabezas} permiten atender múltiples tipos de bordes en paralelo
\end{itemize}

\column{0.42\textwidth}
\begin{block}{¿Qué logra esto?}
Los features RGB \textit{preguntan} al mapa de bordes dónde están los contornos anatómicos y enriquecen su representación antes del decoder.
\end{block}
\end{columns}
\end{frame}

% ── Slide 19: Gate aprendible ─────────────────────────────────────────────────
\begin{frame}{Gate Escalar Aprendible}
\begin{columns}
\column{0.55\textwidth}
\begin{equation*}
\alpha = \sigma(\theta), \quad \theta_0 = -4 \Rightarrow \alpha_0 \approx 0.02
\end{equation*}
\vspace{0.3em}
\begin{itemize}
    \item Al inicio: modelo casi puramente RGB
    \item Durante el entrenamiento: $\alpha$ crece \textbf{solo si los bordes ayudan}
    \item Si el mapa Canny es ruidoso: el gate permanece cerrado
\end{itemize}

\column{0.42\textwidth}
\begin{alertblock}{¿Por qué es importante?}
Sin el gate, los features de bordes ruidosos degradarían el rendimiento. El gate protege al modelo.
\end{alertblock}
\vspace{0.3em}
\begin{block}{Diferencia con Early Fusion}
Early Fusion fuerza ambas modalidades desde la época 1. El gate es selectivo.
\end{block}
\end{columns}
\end{frame}

% ── Slide 20: Fusión multi-escala ─────────────────────────────────────────────
\begin{frame}{Fusión Multi-Escala}
\textbf{Dos instancias de cross-attention:}
\vspace{0.3em}
\begin{enumerate}
    \item \textbf{Cuello de botella} — 512 canales, resolución $16{\times}16$
    \item \textbf{Etapa 4} — 256 canales, resolución $32{\times}32$
\end{enumerate}
\vspace{0.5em}
\textbf{Skip connections del encoder de bordes:}
\begin{equation*}
\hat{\mathbf{F}}^{(s)} = \text{Conv}_{1\times1}\!\left(\left[\mathbf{F}_A^{(s)} \;\|\; \mathbf{F}_B^{(s)}\right]\right), \quad s \in \{1,2,3,4\}
\end{equation*}
\vspace{0.2em}
\begin{block}{Efecto combinado}
Los cues geométricos se propagan por \textbf{todo el camino de expansión}, no solo en el cuello de botella.
\end{block}
\end{frame}

% ── Slide 21: Flujo completo del modelo ──────────────────────────────────────
\begin{frame}{Flujo Completo del Modelo}
\centering
\begin{tikzpicture}[font=\small, node distance=0.45cm and 0.6cm,
    box/.style={draw, rounded corners, minimum width=2.1cm, minimum height=0.55cm, align=center, font=\footnotesize},
    arr/.style={-{Stealth}, thick}]

    % Top stream: RGB
    \node[box, fill=blue!15] (rgb)    {Imagen RGB\\$512^2$};
    \node[box, fill=blue!15, right=of rgb] (encA)  {Encoder RGB\\ResNet-34};
    \node[box, fill=orange!20, right=of encA] (ca)    {Cross-\\Attention $\times2$};
    \node[box, fill=gray!15, right=of ca] (dec)   {Decoder};
    \node[box, fill=green!20, right=of dec] (out)   {Máscara\\$512^2$};

    % Bottom stream: Edges
    \node[box, fill=red!15, below=1.1cm of rgb] (canny) {Canny\\$t_1{=}50,\,t_2{=}150$};
    \node[box, fill=red!15, right=of canny] (encB)  {Encoder Bordes\\ResNet-34};

    % Arrows top stream
    \draw[arr] (rgb)  -- (encA);
    \draw[arr] (encA) -- (ca);
    \draw[arr] (ca)   -- (dec);
    \draw[arr] (dec)  -- (out);

    % Arrow bottom stream
    \draw[arr] (rgb.south) -- (canny.north);
    \draw[arr] (canny) -- (encB);

    % Cross connections
    \draw[arr, red!60, dashed] (encB.north east) -- (ca.south west)
        node[midway, right, font=\tiny] {K, V};
    \draw[arr, blue!60, dashed] (encA.south) -- ++(0,-0.25) -| (ca.south)
        node[near start, below, font=\tiny] {Q};

    % Skip label
    \draw[arr, purple!60, dashed] (encB.east) -- ++(0.3,0) |- (dec.south)
        node[near end, right, font=\tiny] {skips};

\end{tikzpicture}
\vspace{0.5em}

{\small Ambos encoders comparten arquitectura (ResNet-34) pero \textbf{no pesos}. El decoder recibe el bottleneck fusionado más skips de \textbf{ambos} encoders.}
\end{frame}

% ── Slide 22: Configuración de entrenamiento ─────────────────────────────────
\begin{frame}{Configuración de Entrenamiento}
\begin{columns}
\column{0.48\textwidth}
\textbf{Función de pérdida}
\begin{equation*}
\mathcal{L} = \mathcal{L}_\text{BCE} + \mathcal{L}_\text{Dice}
\end{equation*}
Penaliza errores a nivel de píxel y de solapamiento

\vspace{0.4em}
\textbf{Optimizador:} AdamW\\
lr $= 10^{-4}$, weight decay $= 10^{-2}$

\vspace{0.4em}
\textbf{Schedule:} 100 épocas, batch 8

\column{0.48\textwidth}
\textbf{Augmentación (solo entrenamiento)}
\begin{itemize}
    \item Flip horizontal ($p = 0.5$)
    \item Rotación $\pm 15°$
    \item Jitter de brillo/contraste $[0.8, 1.2]$
    \item Mapa de bordes \textbf{recalculado} tras transforms geométricas
\end{itemize}

\vspace{0.3em}
\textbf{Hardware:} NVIDIA A100 40GB (Colab)\\
\textbf{Semilla:} 42 (todas las corridas)
\end{columns}
\end{frame}

% ══════════════════════════════════════════════════════════════════════════════
%  EXPERIMENTOS Y RESULTADOS — Slides 23–29
% ══════════════════════════════════════════════════════════════════════════════

% ── Slide 23: Dataset ─────────────────────────────────────────────────────────
\begin{frame}{Dataset: Cataract-Seg}
\begin{columns}
\column{0.5\textwidth}
\begin{itemize}
    \item \textbf{Fuente:} Roboflow Universe (Muhammad Risma, 2023)
    \item Imágenes de segmento anterior con máscaras de píxeles
    \item Estructuras etiquetadas: córnea, pupila, cristalino
    \item \textbf{300 imágenes} en total
\end{itemize}
\vspace{0.4em}
\textbf{Partición estratificada 70/15/15:}
\begin{center}
\begin{tabular}{lc}
\hline
Split & Imágenes \\
\hline
Entrenamiento & 210 \\
Validación    & 45  \\
\textbf{Test} & \textbf{45}  \\
\hline
\end{tabular}
\end{center}

\column{0.46\textwidth}
\begin{block}{Reto del dataset}
300 imágenes es pequeño para segmentación médica. Los modelos fundacionales como MedSAM requieren órdenes de magnitud más datos — de ahí la apuesta por un modelo liviano sin preentrenamiento.
\end{block}
\vspace{0.3em}
{\small Augmentación geométrica y fotométrica para mitigar el tamaño reducido.}
\end{columns}
\end{frame}

% ── Slide 24: Diseño experimental ────────────────────────────────────────────
\begin{frame}{Diseño Experimental: ¿Qué Comparamos?}
\begin{columns}[T]
\column{0.52\textwidth}
Cuatro modelos con \textbf{mismo backbone, optimizador y schedule}:
\vspace{0.3em}
\begin{itemize}
    \item \textbf{U-Net RGB} — baseline unimodal
    \item \textbf{U-Net Bordes} — Canny solo, sin color
    \item \textbf{Fusión Temprana} — concatenación de canales en la entrada
    \item \textbf{Propuesto} — encoder dual + cross-attention
\end{itemize}
\vspace{0.3em}
{\small La comparación es una \textbf{ablación implícita}: cada modelo aisla la contribución de una decisión de diseño.}

\column{0.44\textwidth}
\begin{block}{Pregunta de investigación}
¿Agrega valor la fusión por cross-attention respecto a simplemente concatenar las modalidades en la entrada?
\end{block}
\vspace{0.3em}
\begin{alertblock}{Limitación}
Sin ablación formal del gate $\alpha$ ni de las skip connections duales por separado.
\end{alertblock}
\end{columns}
\end{frame}

% ── Slide 25: Métricas de Evaluación ─────────────────────────────────────────
\begin{frame}{Métricas de Evaluación}
\begin{columns}[T]
\column{0.48\textwidth}
\textbf{IoU} — métrica primaria
\begin{equation*}
    \text{IoU} = \frac{|\hat{M} \cap M|}{|\hat{M} \cup M|}
\end{equation*}
{\small Solapamiento / área total. Penaliza FP y FN.
Reportamos el promedio sobre las 45 imágenes del test set.}

\includegraphics[width=0.55\textwidth]{../figures/iou_comparison.pdf}

\column{0.48\textwidth}
\textbf{Dice / F1} — métrica secundaria
\begin{equation*}
    \text{Dice} = \frac{2|\hat{M} \cap M|}{|\hat{M}| + |M|}
\end{equation*}
{\small Equivalente a F1. Más sensible a solapamientos parciales que IoU.}
\vspace{0.4em}

\begin{block}{Relación}
IoU es más \textbf{conservador}: $\text{IoU} \leq \text{Dice}$.\\
Si IoU=0.95, Dice$\approx$0.974.
\end{block}
\end{columns}
\end{frame}

% ── Slide 25: Resultados cuantitativos ───────────────────────────────────────
\begin{frame}{Resultados Cuantitativos}
\centering
\begin{tabular}{lcc}
\hline
\textbf{Modelo} & \textbf{IoU} & \textbf{Dice/F1} \\
\hline
U-Net (RGB)            & 0.9529 & 0.9759 \\
U-Net (Bordes)         & 0.8817 & 0.9371 \\
U-Net (Fusión T.)      & \textbf{0.9532} & \textbf{0.9761} \\
\textbf{Propuesto}     & 0.9530 & 0.9759 \\
\hline
\end{tabular}
\vspace{0.5em}
\begin{itemize}
    \item Los bordes solos confirman que Canny no es suficiente de forma independiente (IoU 0.88)
    \item Los 3 modelos basados en RGB están dentro de \textbf{0.0003 IoU} — ruido estadístico a $n=45$
    \item El modelo propuesto \textbf{empata en primer lugar} con un mecanismo más principiado
\end{itemize}
\end{frame}

% ── Slide 26: Gráfico IoU ─────────────────────────────────────────────────────
\begin{frame}{Comparación de IoU en Test}
\centering
\includegraphics[width=0.75\textwidth]{../paper/figures/iou_comparison.pdf}
\vspace{0.3em}

{\small Eje Y desde 0.85 para ampliar diferencias. La brecha principal es entre bordes solos y los modelos RGB.}
\end{frame}

% ── Slide 27: Curvas de pérdida ──────────────────────────────────────────────
\begin{frame}{Curvas de Pérdida}
\centering
\includegraphics[width=0.85\textwidth]{../paper/figures/loss_curves.pdf}
\vspace{0.3em}

{\small Pérdida BCE+Dice de entrenamiento (izq.) y validación (der.) — 100 épocas. Todos los modelos convergen sin divergencia.}
\end{frame}

% ── Slide 28: Curvas de IoU de validación ────────────────────────────────────
\begin{frame}{IoU de Validación por Época}
\centering
\includegraphics[width=0.82\textwidth]{../paper/figures/val_iou_curves.pdf}
\vspace{0.3em}

{\small El modelo propuesto y Fusión Temprana siguen trayectorias paralelas. Bordes solos: meseta más baja.}
\end{frame}

% ── Slide 29: Predicciones cualitativas ──────────────────────────────────────
\begin{frame}{Predicciones Cualitativas}
\centering
\includegraphics[width=0.82\textwidth]{../paper/figures/qualitative_predictions.pdf}

{\small \textbf{Columnas:} RGB — GT — U-Net RGB — U-Net Bordes — Fusión T. — \textbf{Propuesto}\\[0.1em]
Bordes solos: contornos más gruesos y ruidosos. Los modelos RGB: máscaras visualmente similares.}
\end{frame}

% ══════════════════════════════════════════════════════════════════════════════
%  CONCLUSIÓN — Slides 30–33
% ══════════════════════════════════════════════════════════════════════════════

% ── Slide 30: Conclusión — título ────────────────────────────────────────────
\begin{frame}{Conclusión}
\centering
\vspace{1em}
{\Large Tres contribuciones arquitectónicas.\\[0.5em] Un resultado competitivo.}
\end{frame}

% ── Slide 31: Lo que hicimos ──────────────────────────────────────────────────
\begin{frame}{Conclusión: Lo que Hicimos}
\begin{itemize}
    \item Encoder dual U-Net: RGB + mapa de bordes Canny
    \vspace{0.4em}
    \item Cross-attention con gate aprendible en cuello de botella + etapa 4
    \vspace{0.4em}
    \item Skip connections del encoder de bordes en todos los niveles del decodificador
\end{itemize}
\vspace{0.5em}
\begin{alertblock}{Limitaciones}
\begin{itemize}
    \item Test set de 45 imágenes — diferencias dentro del ruido estadístico
\end{itemize}
\end{alertblock}
\end{frame}

% ── Slide 32: Lo que logramos ─────────────────────────────────────────────────
\begin{frame}{Conclusión: Lo que Logramos}
\begin{itemize}
    \item \textbf{0.9530 IoU} — empatado con el mejor baseline
    \vspace{0.4em}
    \item Fusión robusta al ruido gracias al gate aprendible
    \vspace{0.4em}
    \item Reproducible: A100, semilla 42, código en GitHub
\end{itemize}
\end{frame}

% ── Slide 33: Gracias ─────────────────────────────────────────────────────────
\begin{frame}{}
\centering
\vspace{2em}
{\LARGE \textbf{¡Gracias!}}\\[1.5em]
{\large ¿Preguntas?}\\[2em]
\texttt{github.com/DaniloDuque/multimodal-cataract-seg}
\end{frame}

\end{document}
