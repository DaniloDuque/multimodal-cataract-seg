...
The fused skip connections $\hat{\mathbf{F}}^{(s)}$ replace the plain RGB skips at each resolution level. A final $1{\times}1$ convolution with sigmoid activation produces the binary segmentation mask $\hat{\mathbf{M}} \in [0,1]^{H \times W}$.
...
%
\begin{equation}
\mathcal{L}_\text{Dice} = 1 - \frac{2\sum_{i} \hat{M}_i \cdot M_i}{\sum_{i} \hat{M}_i + \sum_{i} M_i}
\end{equation}
%
